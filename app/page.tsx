...
export default function DeepMediaAnalyzerApp() {
  ...
  const [modelName, setModelName] = React.useState(
    "Gemini 3.7 Flash (Forensic Engine)",
  );
    ...
    "session-1": [
      {
        ...
        role: "assistant",
        content:
          "Welcome to VeritasAI Deep Media Analyzer. I am powered by Gemini 3.7 Flash and client-side digital forensics. Upload an image, audio clip, or video file to run synthetic risk analysis, or ask questions about digital media integrity.",
        timestamp: new Date().toISOString(),
        analysisData: {
          syntheticRisk: 92,
          verdict: "Synthetic",
          summary:
            "Analysis of benchmark sample indicates 92% synthetic probability.",
          ...
          metadataIndicators:
            BENCHMARK_SAMPLES[0].xaiReasoning.metadataIndicators,
          exifData: BENCHMARK_SAMPLES[0].exifData,
          groundingSources: [
            {
              title: "Google Gemini Search Grounding Verification",
              url: "https://gemini.google.com",
            },
            {
              title: "C2PA Content Credentials Verification Standard",
              url: "https://c2pa.org",
            },
          ],
        },
  ...
  const activeSessionTitle =
    sessions.find((s) => s.id === activeSessionId)?.title || "Forensic Audit";
  ...
  const handleSendMessage = async (
    ...
    const userMsg: ChatMessage = {
      ...
      role: "user",
      content:
        text || (attachedFile ? `Analyze file: ${attachedFile.name}` : ""),
      timestamp: new Date().toISOString(),
          ...
          ? {
              ...
              exifData:
                Object.keys(clientExif).length > 0
                  ? clientExif
                  : aiResult.exifData || BENCHMARK_SAMPLES[0].exifData,
